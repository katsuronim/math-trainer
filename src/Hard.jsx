...
function Hard() {
    const [varOne, setVarOne] = useState(Math.floor(Math.random() * 1000));
    const [varTwo, setVarTwo] = useState(Math.floor(Math.random() * 1000));
    ...
    function randomize() {
        setVarOne(Math.floor(Math.random() * 100));
        setVarTwo(Math.floor(Math.random() * 100));
    ...
    return (
        <>
            <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-red-500 to-purple-700 p-6 text-center">
                <Link
                    to="/"
                ...
                <h1 className="text-3xl md:text-5xl font-extrabold text-gray-800 drop-shadow-md mb-6">
                    🎉 Math Trainer 🎉
                </h1>
                ...
                <p className="mt-6 text-2xl font-semibold text-gray-900">🎯 Score: <span className="text-green-400">{point}</span></p>
            </div>  
        </>
